...
class LoginModel
{
    ...
    public static function login($user_name, $user_password, $set_remember_me_cookie = null)
    {
        ...
        if ($set_remember_me_cookie) {
            self::setRememberMeInDatabaseAndCookie($result->uid);
        }

        // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
        self::setSuccessfulLoginIntoSession(
            $result->uid, $result->user_name, $result->nama_sdm, $result->email, $result->user_provider_type, $result->department, $result->grade
        );
    ...
    public static function loginWithCookie($cookie)
    {
        ...
        list ($uid, $token, $hash) = explode(':', $cookie);

        // decrypt user id
        $uid = Encryption::decrypt($uid);

        if ($hash !== hash('sha256', $uid . ':' . $token) OR empty($token) OR empty($uid)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_COOKIE_INVALID'));
            return false;
        }

        // get data of user that has this id and this token
        $result = UserModel::getUserDataByUserIdAndToken($uid, $token);

        // if user with that id and exactly that cookie token exists in database
        if ($result) {

            // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
            self::setSuccessfulLoginIntoSession($result->uid, $result->user_name, $result->nama_sdm, $result->email, $result->user_provider_type, $result->department, $result->grade);
     ...
     */
    public static function setSuccessfulLoginIntoSession($uid, $user_name, $nama_sdm, $email, $user_provider_type, $department, $grade)
    {
        Session::init();
        ...
        // get and set avatars
        Session::set('user_avatar_file', AvatarModel::getPublicUserAvatarFilePathByUserId($uid));
        Session::set('user_gravatar_image_url', AvatarModel::getGravatarLinkByEmail($email));

        // finally, set user as logged-in
        Session::set('user_logged_in', true);

        // update session id in database
        Session::updateSessionId($uid, session_id());
     ...
     * @param $uid
     */
    public static function setRememberMeInDatabaseAndCookie($uid)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        ...
        $sql = "UPDATE users SET user_remember_me_token = :user_remember_me_token WHERE uid = :uid LIMIT 1";
        $sth = $database->prepare($sql);
        $sth->execute(array(':user_remember_me_token' => $random_token_string, ':uid' => $uid));

        // generate cookie string that consists of user id, random string and combined hash of both
        // never expose the original user id, instead, encrypt it.
        $cookie_string_first_part = Encryption::encrypt($uid) . ':' . $random_token_string;
        $cookie_string_hash       = hash('sha256', $uid . ':' . $random_token_string);
     ...
     * @param string $uid
     */
    public static function deleteCookie($uid = null)
    {
        // is $uid was set, then clear remember_me token in database
        if (isset($uid)) {
            ...
            $sql = "UPDATE users SET user_remember_me_token = :user_remember_me_token WHERE uid = :uid LIMIT 1";
            $sth = $database->prepare($sql);
            $sth->execute(array(':user_remember_me_token' => NULL, ':uid' => $uid));
        }
