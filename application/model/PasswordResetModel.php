...
class PasswordResetModel
{
    ...
    public static function requestPasswordReset($user_name_or_email, $captcha)
    {
        ...
        $result = UserModel::getUserDataByLogin($user_name_or_email);
        if (!$result) {
        ...
        $token_set = self::setPasswordResetDatabaseToken($result->user_name, $result->email, $user_password_reset_hash, $temporary_timestamp);
        if (!$token_set) {
            return false;
        }

        // ... and send a mail to the user, containing a link with username and token hash string
        $mail_sent = self::sendPasswordResetMail($result->user_name, $user_password_reset_hash, $result->email);
        if ($mail_sent) {
            return true;
        }
     ...
     * @return bool success status
     */
    public static function setPasswordResetDatabaseToken($user_name, $email, $user_password_reset_hash, $temporary_timestamp)
    {
        ...
        $sql = "UPDATE users
                ...
                WHERE user_name = :user_name AND email = :email AND user_provider_type = :provider_type LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_reset_hash' => $user_password_reset_hash, ':user_name' => $user_name, ':email' => $email, ':user_password_reset_timestamp' => $temporary_timestamp, ':provider_type' => 'employee'
        ));
     ...
     * @return bool success status
     */
    public static function sendPasswordResetMail($user_name, $user_password_reset_hash, $email)
    {
        // create email body
        ...
        $mail = new Mail;
        $mail_sent = $mail->sendMail($email, Config::get('EMAIL_PASSWORD_RESET_FROM_EMAIL'),
    ...
    public static function verifyPasswordReset($user_name, $verification_code)
    {
        ...
        $sql = "SELECT uid, user_password_reset_timestamp
                  FROM users
    ...
    public static function saveNewUserPassword($user_name, $user_password_hash, $user_password_reset_hash)
    {
        ...
        $sql = "UPDATE users SET user_password_hash = :user_password_hash, user_password_reset_hash = NULL,
                 ...
                 WHERE user_name = :user_name AND user_password_reset_hash = :user_password_reset_hash LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(
    ...
    public static function validatePasswordChange($user_id, $user_password_current, $user_password_new, $user_password_repeat)
    {
        ...
        $sql = "SELECT `user_password_hash`, `user_failed_logins` FROM `users` WHERE `uid` = :user_id LIMIT 1;";
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_id' => $user_id
        ));
