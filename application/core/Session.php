...
class Session
{
    ...
    /**
     * unsets a specific value to a specific key of the session
     *
     * @param mixed $key key
     * @param mixed $value value
     */
    ...
    public static function updateSessionId($userId, $sessionId = null)
    {
        ...
        $sql = "UPDATE users SET session_id = :session_id WHERE uid = :uid";

        $query = $database->prepare($sql);
        $query->execute(array(':session_id' => $sessionId, ":uid" => $userId));
    }
    ...
    public static function isConcurrentSessionExists()
    {
        ...
        if (isset($userId) && isset($session_id)) {
            ...
            $database = DatabaseFactory::getFactory()->getConnection();
            $sql = "SELECT session_id FROM users WHERE uid = :uid LIMIT 1";
